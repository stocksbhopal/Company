package com.sanjoyghosh.company.source.yahoo;

public class YahooAnalystOpinion {

	private Double meanRecommendationThisWeek;
	private Double meanRecommendationLastWeek;
	private Double changeMeanRecommendation;
	private Double meanTarget;
	private Double medianTarget;
	private Double highTarget;
	private Double lowTarget;
	private Integer numberOfBrokers;
	private Integer strongBuyCurrentMonth;
	private Integer strongBuyLastMonth;
	private Integer strongBuyTwoMonthsAgo;
	private Integer strongBuyThreeMonthsAgo;
	private Integer buyCurrentMonth;
	private Integer buyLastMonth;
	private Integer buyTwoMonthsAgo;
	private Integer buyThreeMonthsAgo;
	private Integer holdCurrentMonth;
	private Integer holdLastMonth;
	private Integer holdTwoMonthsAgo;
	private Integer holdThreeMonthsAgo;
	private Integer underperformCurrentMonth;
	private Integer underperformLastMonth;
	private Integer underperformTwoMonthsAgo;
	private Integer underperformThreeMonthsAgo;
	private Integer sellCurrentMonth;
	private Integer sellLastMonth;
	private Integer sellTwoMonthsAgo;
	private Integer sellThreeMonthsAgo;

	
	public YahooAnalystOpinion() {}


	public Double getMeanRecommendationThisWeek() {
		return meanRecommendationThisWeek;
	}


	public void setMeanRecommendationThisWeek(Double meanRecommendationThisWeek) {
		this.meanRecommendationThisWeek = meanRecommendationThisWeek;
	}


	public Double getMeanRecommendationLastWeek() {
		return meanRecommendationLastWeek;
	}


	public void setMeanRecommendationLastWeek(Double meanRecommendationLastWeek) {
		this.meanRecommendationLastWeek = meanRecommendationLastWeek;
	}


	public Double getChangeMeanRecommendation() {
		return changeMeanRecommendation;
	}


	public void setChangeMeanRecommendation(Double changeMeanRecommendation) {
		this.changeMeanRecommendation = changeMeanRecommendation;
	}


	public Double getMeanTarget() {
		return meanTarget;
	}


	public void setMeanTarget(Double meanTarget) {
		this.meanTarget = meanTarget;
	}


	public Double getMedianTarget() {
		return medianTarget;
	}


	public void setMedianTarget(Double medianTarget) {
		this.medianTarget = medianTarget;
	}


	public Double getHighTarget() {
		return highTarget;
	}


	public void setHighTarget(Double highTarget) {
		this.highTarget = highTarget;
	}


	public Double getLowTarget() {
		return lowTarget;
	}


	public void setLowTarget(Double lowTarget) {
		this.lowTarget = lowTarget;
	}


	public Integer getNumberOfBrokers() {
		return numberOfBrokers;
	}


	public void setNumberOfBrokers(Integer numberOfBrokers) {
		this.numberOfBrokers = numberOfBrokers;
	}


	public Integer getStrongBuyCurrentMonth() {
		return strongBuyCurrentMonth;
	}


	public void setStrongBuyCurrentMonth(Integer strongBuyCurrentMonth) {
		this.strongBuyCurrentMonth = strongBuyCurrentMonth;
	}


	public Integer getStrongBuyLastMonth() {
		return strongBuyLastMonth;
	}


	public void setStrongBuyLastMonth(Integer strongBuyLastMonth) {
		this.strongBuyLastMonth = strongBuyLastMonth;
	}


	public Integer getStrongBuyTwoMonthsAgo() {
		return strongBuyTwoMonthsAgo;
	}


	public void setStrongBuyTwoMonthsAgo(Integer strongBuyTwoMonthsAgo) {
		this.strongBuyTwoMonthsAgo = strongBuyTwoMonthsAgo;
	}


	public Integer getStrongBuyThreeMonthsAgo() {
		return strongBuyThreeMonthsAgo;
	}


	public void setStrongBuyThreeMonthsAgo(Integer strongBuyThreeMonthsAgo) {
		this.strongBuyThreeMonthsAgo = strongBuyThreeMonthsAgo;
	}


	public Integer getBuyCurrentMonth() {
		return buyCurrentMonth;
	}


	public void setBuyCurrentMonth(Integer buyCurrentMonth) {
		this.buyCurrentMonth = buyCurrentMonth;
	}


	public Integer getBuyLastMonth() {
		return buyLastMonth;
	}


	public void setBuyLastMonth(Integer buyLastMonth) {
		this.buyLastMonth = buyLastMonth;
	}


	public Integer getBuyTwoMonthsAgo() {
		return buyTwoMonthsAgo;
	}


	public void setBuyTwoMonthsAgo(Integer buyTwoMonthsAgo) {
		this.buyTwoMonthsAgo = buyTwoMonthsAgo;
	}


	public Integer getBuyThreeMonthsAgo() {
		return buyThreeMonthsAgo;
	}


	public void setBuyThreeMonthsAgo(Integer buyThreeMonthsAgo) {
		this.buyThreeMonthsAgo = buyThreeMonthsAgo;
	}


	public Integer getHoldCurrentMonth() {
		return holdCurrentMonth;
	}


	public void setHoldCurrentMonth(Integer holdCurrentMonth) {
		this.holdCurrentMonth = holdCurrentMonth;
	}


	public Integer getHoldLastMonth() {
		return holdLastMonth;
	}


	public void setHoldLastMonth(Integer holdLastMonth) {
		this.holdLastMonth = holdLastMonth;
	}


	public Integer getHoldTwoMonthsAgo() {
		return holdTwoMonthsAgo;
	}


	public void setHoldTwoMonthsAgo(Integer holdTwoMonthsAgo) {
		this.holdTwoMonthsAgo = holdTwoMonthsAgo;
	}


	public Integer getHoldThreeMonthsAgo() {
		return holdThreeMonthsAgo;
	}


	public void setHoldThreeMonthsAgo(Integer holdThreeMonthsAgo) {
		this.holdThreeMonthsAgo = holdThreeMonthsAgo;
	}


	public Integer getUnderperformCurrentMonth() {
		return underperformCurrentMonth;
	}


	public void setUnderperformCurrentMonth(Integer underperformCurrentMonth) {
		this.underperformCurrentMonth = underperformCurrentMonth;
	}


	public Integer getUnderperformLastMonth() {
		return underperformLastMonth;
	}


	public void setUnderperformLastMonth(Integer underperformLastMonth) {
		this.underperformLastMonth = underperformLastMonth;
	}


	public Integer getUnderperformTwoMonthsAgo() {
		return underperformTwoMonthsAgo;
	}


	public void setUnderperformTwoMonthsAgo(Integer underperformTwoMonthsAgo) {
		this.underperformTwoMonthsAgo = underperformTwoMonthsAgo;
	}


	public Integer getUnderperformThreeMonthsAgo() {
		return underperformThreeMonthsAgo;
	}


	public void setUnderperformThreeMonthsAgo(Integer underperformThreeMonthsAgo) {
		this.underperformThreeMonthsAgo = underperformThreeMonthsAgo;
	}


	public Integer getSellCurrentMonth() {
		return sellCurrentMonth;
	}


	public void setSellCurrentMonth(Integer sellCurrentMonth) {
		this.sellCurrentMonth = sellCurrentMonth;
	}


	public Integer getSellLastMonth() {
		return sellLastMonth;
	}


	public void setSellLastMonth(Integer sellLastMonth) {
		this.sellLastMonth = sellLastMonth;
	}


	public Integer getSellTwoMonthsAgo() {
		return sellTwoMonthsAgo;
	}


	public void setSellTwoMonthsAgo(Integer sellTwoMonthsAgo) {
		this.sellTwoMonthsAgo = sellTwoMonthsAgo;
	}


	public Integer getSellThreeMonthsAgo() {
		return sellThreeMonthsAgo;
	}


	public void setSellThreeMonthsAgo(Integer sellThreeMonthsAgo) {
		this.sellThreeMonthsAgo = sellThreeMonthsAgo;
	}


	@Override
	public String toString() {
		return "YahooAnalystOpinion [meanRecommendationThisWeek=" + meanRecommendationThisWeek
				+ ", meanRecommendationLastWeek=" + meanRecommendationLastWeek + ", changeMeanRecommendation="
				+ changeMeanRecommendation + ", meanTarget=" + meanTarget + ", medianTarget=" + medianTarget
				+ ", highTarget=" + highTarget + ", lowTarget=" + lowTarget + ", numberOfBrokers=" + numberOfBrokers
				+ ", strongBuyCurrentMonth=" + strongBuyCurrentMonth + ", strongBuyLastMonth=" + strongBuyLastMonth
				+ ", strongBuyTwoMonthsAgo=" + strongBuyTwoMonthsAgo + ", strongBuyThreeMonthsAgo="
				+ strongBuyThreeMonthsAgo + ", buyCurrentMonth=" + buyCurrentMonth + ", buyLastMonth=" + buyLastMonth
				+ ", buyTwoMonthsAgo=" + buyTwoMonthsAgo + ", buyThreeMonthsAgo=" + buyThreeMonthsAgo
				+ ", holdCurrentMonth=" + holdCurrentMonth + ", holdLastMonth=" + holdLastMonth + ", holdTwoMonthsAgo="
				+ holdTwoMonthsAgo + ", holdThreeMonthsAgo=" + holdThreeMonthsAgo + ", underperformCurrentMonth="
				+ underperformCurrentMonth + ", underperformLastMonth=" + underperformLastMonth
				+ ", underperformTwoMonthsAgo=" + underperformTwoMonthsAgo + ", underperformThreeMonthsAgo="
				+ underperformThreeMonthsAgo + ", sellCurrentMonth=" + sellCurrentMonth + ", sellLastMonth="
				+ sellLastMonth + ", sellTwoMonthsAgo=" + sellTwoMonthsAgo + ", sellThreeMonthsAgo="
				+ sellThreeMonthsAgo + "]";
	}
}
